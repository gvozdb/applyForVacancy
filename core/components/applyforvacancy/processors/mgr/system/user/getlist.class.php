...
class afvUserGetListProcessor extends modUserGetListProcessor
{
    ...
    public function prepareQueryBeforeCount(xPDOQuery $c)
    {
        ...
        // Фильтр
        foreach (array('position', 'vessel', 'country') as $v) {
    ...
    public function getCountry($key = '')
    {
        if (empty($this->country_lang)) {
            ...
            if ($this->modx->getOption('manager_language') != 'en' &&
                file_exists($this->modx->getOption('core_path') .
                            'lexicon/country/' .
...
return 'afvUserGetListProcessor';