...
class afvTools
{
    ...
    public function __construct(modX &$modx, &$config)
    {
        ...
        if (!is_object($this->modx->applyforvacancy)) {
    ...
    public function runProcessor($action = '', $data = array())
    {
        ...
        $processorsPath = !empty($this->afv->config['processorsPath']) ? $this->afv->config['processorsPath']
            : MODX_CORE_PATH;

        /* @var modProcessorResponse $response */
        $response = $this->modx->runProcessor($action, $data, array('processors_path' => $processorsPath));

        return $this->afv->config['prepareResponse'] ? $this->prepareResponse($response) : $response;
    }
    ...
    public function prepareResponse($response)
    {
        ...
        if ($this->afv->config['jsonResponse'] AND is_array($output)) {
            $output = $this->modx->toJSON($output);
        } elseif (!$this->afv->config['jsonResponse'] AND !is_array($output)) {
    ...
    {
        if ($pdoTools = $this->afv->getPdoTools()) {
    ...
    public function success($message = '', $data = array(), $placeholders = array())
    {
        ...
        return $this->afv->config['jsonResponse'] ? $this->modx->toJSON($response) : $response;
    }
